...
{
    private List<PlannedFile>? approvedPlan;
    ...
    public async Task DeployAsync(UserSettings s,bool upgradeApproved,CancellationToken token=default)
    {
        ...
        if(hardware.Driver is null)throw new InvalidOperationException("驱动检测失败；没有把未知状态当成兼容。");
        if(manifest.FixedMinimumDriver is int minimum && hardware.Driver<minimum)throw new InvalidOperationException("驱动不满足该文件包声明的 Fixed 条件。");
        if(hardware.HagsConfigured==false)throw new InvalidOperationException("HAGS 已配置关闭。请在 Windows 中处理并重启后再检查；工具不会擅自修改系统。");
        bool dynamic=manifest.PreferDynamic&&hardware.Driver>=manifest.DynamicMinimumDriver;
        ...
        var plan=await PackageReader.PlanAsync(manifest,s.PackageManifest,s.GameRoot,Path.GetDirectoryName(exe)!,before.AddonDirectory,token);
        if(approvedPlan is null || !approvedPlan.SequenceEqual(plan))throw new IOException("文件计划与确认时不同或未经预览，请重新预览并确认。");
        approvedPlan=null;
        ...
        var receipt=previousReceipt is not null && previousReceipt.Status!="Cleaned"?previousReceipt:new(){GameRoot=Path.GetFullPath(s.GameRoot),GameExe=exe};
