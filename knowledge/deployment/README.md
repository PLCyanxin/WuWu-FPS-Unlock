...
## 2026-09-23 — executed isolated real-binary acceptance

Read installed wuwa-release skill and protocol before testing. Built the native self-contained updater using .tools/dotnet SDK 10.0.100. Actual isolated installation: artifacts/update-acceptance/installation-cc88bb46671640e39b0c09862eb7a1c3. Baseline was the real 1.1RC package from artifacts/online-v1.1.1/baseline/extracted; destination payload was 1.1.1 from layout-final-v1.1.1-20260923-152029/update. Updated all seven declared payload targets, verified each target hash, preserved an independent old-state snapshot, deleted only the test update folder, ran the backup's own updater to roll back, verified all 32 original non-data file hashes, preserved a newer deployment record and later-added material, then successfully repeated rollback.

Real desktop COM integration created 鸣潮 FPS Unlock.lnk pointing only to this isolated installation, refreshed it on both rollbacks, then the acceptance script removed only that newly created full-path link after matching its target. All 30 pre-existing desktop .lnk files remained byte-identical, including user's 鸣潮.lnk. No D:\software installation or actual game files were modified.

Initial live baseline-WPF attempt could not produce a valid wait assertion: Process.MainModule/Path returned empty for the surviving test window, while QueryFullProcessImageName with limited query rights returned its exact path. The user later closed that test window. Initial harness failures are retained in isolated-update-rollback-initial-harness-failure.log and isolated-update-rollback-module-query-failure.log; do not report these as passed. Subsequent acceptance did not launch WPF again. End-to-end worker exercised the already-exited PID branch, while a real harmless updater child separately exercised native live-image identity and a natural exit wait.

Production fix: worker identity and RejectRunning now use OpenProcess(QUERY_LIMITED_INFORMATION | SYNCHRONIZE), QueryFullProcessImageName and WaitForSingleObject on one held SafeProcessHandle. This avoids unnecessary module-enumeration/VM-read permission requirements and pins identity across the wait. Denied access still rejects; no elevation or termination. --self-test-child sleeps briefly and exits only for the self-test's own isolated process. Latest self-test: 15 passed / 0 failed, including real native child query/wait (not game/launcher execution).

Logs under main artifacts/update-acceptance: updater-build.log; updater-self-tests.log; isolated-update-rollback.log; online-wait-update.log; rollback.log; rollback-repeat.log; package-validation.log; package-retry-validation.log; final-package-validation.log. The final freshly built updater was repackaged with the same real payload and a freshly generated manifest; --validate-package succeeded. Final updater SHA-256 E583A54E13C1CC420D832F805D97B95F2A74F2835052C89429ED2DD05E153B48. The final rebuild after E2E only clarified the self-test summary wording; production path-query logic is the version exercised end to end.

Reusable acceptance script: knowledge/deployment/Invoke-UpdaterAcceptance.ps1 -Workspace <main workspace>; PowerShell 7 required. It uses full path-scoped temporary installations, no game or installed launcher launch, and safely removes only its own package folder/link. User data inside that installation is deliberately synthetic; original package runtime/component/material bytes are real. Actual online GitHub download, GUI button handoff and in-game behavior are separate root-agent validations.
