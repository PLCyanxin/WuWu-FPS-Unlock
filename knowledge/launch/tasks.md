...
## Built-in FPS v0.1 restart work (latest user direction supersedes external unlock route)

Merged current main into the SAME launch agent workspace; did not create a replacement agent/context. Added only RestartController.cs, WindowsRestartProcessCatalog.cs and tests/Restart.ProcessTests. No VM/project/FPS implementation changes in this task.

Integration: keep one RestartController instance for the VM. Call RunAsync(RestartRequest(exactShippingPath,FpsEnabled), new WindowsRestartProcessCatalog(), RestartActions, state, stopTimeout, token). Actions are Preflight (read-only; runs both before and after notice), ConfirmNotice (identities passed before any termination), ReleaseOldSession, Start(canonicalExecutablePath,token), optional AttachFps(process,token). Return null means notice was declined. Return Process is the one started by Start. FPS=false never calls AttachFps. Failures return RestartFailureException with phase, terminated PID list and StartedProcess when FPS attach failed; do not kill that process automatically. Caller owns the returned/exception process handle.

Windows catalog compares canonical full executable paths obtained through file handles. It reads same-name candidates with QUERY_LIMITED_INFORMATION and stops if any candidate is unreadable; other readable paths are ignored without acquiring terminate rights. Matching candidates obtain a held QUERY+TERMINATE+SYNCHRONIZE process handle. Creation time/path are verified while the original query handle is held; all matching handles are verified again after notice before any kill. The SAME held handle is used for final verification, TerminateProcess and exit wait, never reopened by PID for killing. Natural exit during notice/identity changes/terminate denial/wait timeout stop the restart. A fresh matching instance after termination blocks start and is not killed. No UAC or process-memory access.

Actual run: tests/Restart.ProcessTests, SDK10.0.100, 20 passed0failed, no compiler warnings. Real fixture apphosts copied to Chinese/space-containing temporary directories cover no-process launch, exact-path restart, preserving same-name other installations, FPS attach callback order, notice cancel, missing preflight, two matching instances, natural-exit handle race, attach failure leaving new process alive, repeated workflow recovery. Unreadable paths/denied termination/timeouts/PID identity change/new concurrent target are injected fake boundaries; these tests do not claim an actual protected process was terminated. Log at .agent-worktrees/launch/artifacts/restart-tests.log. Only self-built tracked test children were started/terminated; no actual game, FPS DLL, unlocker or UAC ran.

Scope is exact selected Shipping image, not every EXE sharing its directory. The caller's Start delegate must execute its provided canonical path exactly once and must not reroute to external unlocker or a second launcher. OS-level external starts can still race in the tiny interval after the final census; no undocumented global game lock or privilege bypass is attempted.

## Natural-exit refinement

Latest refinement supersedes the earlier strict natural-exit refusal: a signaled SAME held process handle proves that exact kernel process is already stopped, so VerifyStillSameProcess and TerminateAndWaitAsync return without another termination. The terminate failure branch also accepts only that same handle becoming signaled; no PID reopen/second kill. Enumeration can skip a failed open only when a synchronize-only handle is signaled, or Windows reports invalidPID and a fresh process snapshot confirms absence; access denial/unknown identity still blocks. New living same-path instances remain a race and block replacement.

If cancellation occurs after Start returned a process (including inside FPS attachment), RestartFailureException now retains StartedProcess and the original OperationCanceledException for the VM to continue monitoring it. No cancellation cleanup kills it.

Actual regression rerun:23 passed0failed, no warnings, artifacts/restart-tests.log. Added actual owned fixture natural exit while notice is pending, already-exited held-handle completion, cancellation immediately after start, and FPS attachment cancellation retaining a live process reference. No real game/FPS/UAC execution.

## Real FPS failure audit 17:30–17:41 and diagnostic correction

Read only the actual main artifacts/desktop-runtime-20260914-172011/data/logs files20260914-173317,173829,173916. Observed17:36:07 PID28552 FPS160 then17:36:41 AttachingFps access denied;17:39:47 PID46796 then17:40:19 same;17:40:26 PID36588 then17:41:03 same. No IPC-connected or FPS-sent success occurred. Repeated17:38 Start attempts were explicitly rejected by the deployment/config preflight, not a silent button or duplicate-game launch. Existing error text cannot distinguish renderer-path access from module enumeration/injection access. There is NO log evidence of the existing birth-time mismatch message; do not claim PID creation-time caused the actual failure.

Changes limited to BuiltinFpsService/FpsSession and diagnostic tests. Builtin renderer observation holds a read-only identity handle and compares raw FILETIME/PID/full path without rounding or local-time conversions. FpsSession preserves the existing module-reuse order, requested permissions and loading operations; added stage markers and wrapped native error code/PID/phase across renderer identity, module enumeration, access request, birth check, system-loader lookup, allocation/loading verification and IPC. A genuine access denial remains a refusal; no automatic elevation or anti-cheat bypass was added.

Automatic approval rejected one mixed command that combined injector-order adjustments and tests with only 'blocked by policy' stated. That command did not run. Scope was reduced to diagnostic-only injector edits preserving the original operation order, plus pure record/error-format tests. The narrower work executed successfully; no rejected action was reissued via another mechanism.
