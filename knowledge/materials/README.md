...
## Root-to-EXE resolution and authorized directory-name fallback

Synced main3f45c19 or newer. Read-only shortcut resolution through Windows DesktopDirectory and WScript.Shell showed Desktop/鸣潮 FPS Unlock v0.1.lnk points to E:/yanxin_ws/wuwa-fps-unlock/artifacts/desktop-runtime-20260914-172011/WuWaFpsUnlock.exe. That runtime's data/settings.json (SHA25694017275D9CABCB53654DABC15EF72DC56CB278DEB268A31AC296F533CDF3B00) binds root D:/Wuthering Waves/Wuthering Waves Game and EXE Client/Binaries/Win64/Client-Win64-Shipping.exe relative to root. No shortcut or settings were changed; no game launched.

New public GameDiscoveryService.TryResolveGameRoot(root,out candidate,out reason) derives and verifies this exact EXE directly. Public ShippingRelativePath provides the stable relative path. GameDiscoveryCandidate.RelativeShippingExePath records its root-relative EXE. DiscoverAsync now returns a valid current root immediately without enumerating saved hints/registry/disks, as requested; otherwise existing bounded saved/registry/Epic hint search remains. Only if hints find nothing and includeSystemHints=true on Windows does it enumerate ready local Fixed drives and run name search for Wuthering Waves Game / Wuthering Waves.

SearchDirectoriesByName(searchRoots,directoryNames,GameDirectorySearchOptions?,CancellationToken) is separately testable, BFS over directories only, no links followed, exact folder-name matches revalidated with canonical Shipping. Default limits:100000 discovered/checked directory budget, depth16,20seconds, pending queue20000;64 explicit search-root inputs maximum. Each loop checks cancellation; OS filesystem calls themselves cannot be interrupted mid-call, so time cap is cooperative. Access failures, skipped links/depth and capped/incomplete results are reported, not presented as exhaustive no-game findings. This agent ran only temporary-directory tests, not the real fixed-drive fallback.

Seven new root-resolution/name-search regressions pass: direct correct root+relative path, no saved/system hint enumeration on direct success, matching official folders at different depths, false folder names/layouts rejected, directory/depth caps with incomplete diagnostics, cancellation. Current full Core108 passed/0 failed (discovery-directory-search-tests.log). VM needs to use TryResolveGameRoot when root is changed and store/update EXE; VM modifications remain parent's responsibility.
