...
更新器不要求旧版本号或文件哈希相同，可用于本项目 GitHub 和网盘分发的旧版本；需要保留原程序名称及目录结构。更新包可以整体放入原启动器目录的子文件夹，程序会检查自身目录及最多三层上级目录中的启动器，多个候选时停止并提示。它更新启动器、内置 FPS 核心与随附许可，并备份被替换的文件；保留配置、部署记录和自行更新的 MFG/ReShade 材料，不自动修改游戏文件。不支持的目录或被占用的文件会明确提示。首次使用请下载下方完整便携版。
