# 最新覆盖规则（v0.1，2026-09-14）
涉及启动器自动更新、更新包、回退或发布时，先读取并遵守 skills/wuwa-release/SKILL.md 及其更新协议；本规则不覆盖用户最新指示。
用户最新要求优先于下方历史条目及CODEX_TASK。FPS内置，不运行/依赖unlock.exe；启动自动按规范完整Shipping路径终止同安装旧实例并等待后启动一次。旧会话释放，新PID连接。运行中按钮可用，busy互斥；终止失败/无法确认身份停止。新部署后首次须知在终止前展示，确认持久化、取消无副作用，no-op不重置。UI v0.1、自有程序集文件0.1.0.0，保留旧配置/记录/来源。游戏就绪后收起通知区，原彩图标，Windows管理折叠。ReShade按所选游戏及来源处理；自动查找保留手选。真实强制结束/内置注入须集中确认，旧外部路线实测不能替代。子agent继续独立worktree及持久knowledge。
