...
| WPF通知区生命周期 | 9/9 | 实际窗口显示、隐藏、图标、恢复；GameReady由测试模拟 |
...
最新托盘补充：主窗口×收起托盘；游戏就绪自动收起；双击恢复；右键退出启动器才真正退出。使用用户最新图标透明版.png原件生成32位ICO，无新增圆角或抠图。实际原生托盘回归9/9，游戏就绪事件仍为测试模拟。


单实例更新：重复打开桌面快捷方式通过同用户命名管道唤回已有窗口，第二个进程随即退出，不弹已运行提示、不启动游戏；已用独立测试进程验证通信及原生窗口恢复。旧的运行中版本需先从托盘退出，再打开更新版。
