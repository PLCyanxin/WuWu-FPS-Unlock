...
Dynamic 最大倍率是 **v1.2RC 的实验性功能**，为 NVIDIA 原生 Dynamic MFG 调度器提供倍率上限，出厂默认 **4x**。实际倍率仍由原生调度器选择，并非固定为 4x。**本版无法热切换；修改后在下一次完整启动游戏时生效。**

此功能需要带 `DynamicMaxMultiplier` 支持的派生 addon。未修改的上游 addon 或其他本地替换版本可能不识别该设置。

## 设置与部署

首次部署时在启动器中设定上限。后续日常调整可直接在游戏内 ReShade 的 **MFG Unlock → Dynamic maximum multiplier** 中完成，保存后完全退出并重新启动游戏，无需再次部署。若重新部署，将以启动器保存的选值覆盖游戏内的选择，请先核对。
...
**已完成一台设备的人工游戏体验验收，暂未报告异常。** 该结果不代表所有设备均兼容，也不等于已完成全部负载场景的硬上限测量。自动化测试、构建成功和 DRS 日志分别验证不同层次，不能替代游戏内实际倍率与帧间隔的观察。
